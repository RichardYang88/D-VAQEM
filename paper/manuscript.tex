...
mixed depolarising--readout setting and three stronger readout rates, $20$ in
...
$\gamma$ is recovered to $7.3\times10^{-11}$ relative; on depolarising noise,
...
\emph{regularised} one is better: across the $7$ settings with $\gamma>20$ the
Tikhonov term inside the deterministic inverse buys up to $8.9$ dB (dephasing
$p=0.05$, $\gamma=5.4\times10^{10}$).  An unregularised exact inverse is not a
...
(Sec.~\ref{subsec:pec}), so the advantage of knowing the channel is largely
...
\url{https://github.com/RichardYang88/D-VAQEM}; a versioned long-term copy will
be archived at Zenodo.%
% TODO(author): confirm the public repository URL; once the Zenodo snapshot DOI
% is minted, cite it in this footnote in the camera-ready version.  Never ship
% a literal placeholder DOI in the submitted PDF.
\footnote{Long-term archive: the DOI minted for the versioned Zenodo snapshot
will be listed in the camera-ready version.}
